...
mod tests {
    ...
    fn valid_token_round_trips() {
        ...
        let t = mint(secret, &claims("fujin", 10_000));
        assert_eq!(verify(secret, "fujin", 9_999, &t).unwrap().workspace, "ws1");
    }
    ...
    fn rejects_wrong_secret_expiry_and_audience() {
        let secret = b"platform-secret";
        let t = mint(secret, &claims("fujin", 10_000));
        assert!(verify(b"other", "fujin", 1, &t).is_none(), "wrong secret");
        assert!(verify(secret, "fujin", 10_001, &t).is_none(), "expired");
        assert!(verify(secret, "satori", 1, &t).is_none(), "wrong audience");
        assert!(verify(secret, "fujin", 1, "garbage").is_none(), "malformed");
    }
}
